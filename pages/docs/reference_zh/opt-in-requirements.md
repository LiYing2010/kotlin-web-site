...
如果你在代码中使用了某个 API, 而你的代码本身又打算给第三方使用(是一个库), 那么你也可以将这个的 API 的要求使用者同意设定传递给你的 API.
为了做到这一点, 需要将你的函数体中使用到的 API 的 [_明确要求使用者同意(Opt-in Requirement) 注解_](#opt-in-requirement-annotations) 添加到你的函数的声明部分.
...
这种情况下, 请对你的函数声明标注 [@OptIn](/api/latest/jvm/stdlib/kotlin/-opt-in/index.html) 注解,
并在这个注解的参数中指明要求使用者同意的注解:
...
如果你的 Gradle 模块是一个跨平台模块, 请使用 `useExperimentalAnnotation` 方法:
...
这个注解类本身必须标注 [@RequiresOptIn](/api/latest/jvm/stdlib/kotlin/-requires-opt-in/index.html) 注解:
...
* [retention](/api/latest/jvm/stdlib/kotlin.annotation/-annotation-retention/index.html) 设置为 `BINARY`
* [targets](/api/latest/jvm/stdlib/kotlin.annotation/-annotation-target/index.html) 中不含 `EXPRESSION` 和 `FILE`
* 没有参数.

对用户同意要求的严重 [级别](/api/latest/jvm/stdlib/kotlin/-requires-opt-in/-level/index.html) 可以是以下两种之一:
...
如果要删除这些警告, 请添加编译选项 `-Xopt-in=kotlin.RequiresOptIn`.
