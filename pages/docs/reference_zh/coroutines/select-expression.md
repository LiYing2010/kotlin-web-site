...
注意, [onReceiveOrNull][onReceiveOrNull] 是一个扩展函数, 只对元素为非null值的通道有效,
因此, 我们可以明确区分一个已经关闭的通道, 和通道中得到的一个 null 值.
...
第 2 个现象是, 当通道被关闭时, 会立即选择 [onReceiveOrNull][onReceiveOrNull] 子句.
...
这个示例程序在同一个 `select` 中结合使用了 [onReceiveOrNull][onReceiveOrNull] 子句和 [onAwait][Deferred.onAwait] 子句:
