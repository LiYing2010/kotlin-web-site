...
![编译任务]({{ url_for('asset', path='images/reference/mpp/compilations.png') }})
...
## 在 JVM 编译任务中包含 Java 源代码

默认情况下, JVM 编译目标 会忽略 Java 源代码, 只编译 Kotlin 源代码文件.

要将 Java 源代码包含到 JVM 编译目标的编译任务, 需要对编译目标明确开启 Java 语言支持:

* [使用项目向导创建项目](mpp-create-lib.html) 时:

   ![开始 Java 语言支持]({{ url_for('asset', path='images/reference/mpp/enable-java-support.png') }})
...
* 在既有项目的构建脚本中:
  ...
  这个设置会适用 Gradle `java` plugin, 并配置编译目标, 使其与 `java` plugin 协作.

Java 源代码文件 放在 Kotlin 源代码根路径的子目录之内. 比如, 路径是:

<img class="img-responsive" src="{{ url_for('asset', path='images/reference/mpp/java-source-paths.png') }}" alt="Java 源代码文件" width="200"/>
...
Kotlin 提供 [与原生语言的交互能力](native/c_interop.html), 以及对编译任务进行相关配置的 DSL.
...
编译任务可以与几个原生库交互. 在编译任务的 `cinterops` 代码段中, 可以使用 [可用的参数](mpp-dsl-reference.html#cinterops) 来配置交互能力.
...
<img class="img-responsive" src="{{ url_for('asset', path='images/reference/mpp/jvm-js-main.png') }}" alt="源代码集层级结构" width="400"/>
