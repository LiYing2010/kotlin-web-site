...
> [更多详情请参见 Kotlin/JS 中的反射功能](js-reflection.html).
{:.note}
...
指向构造器的引用的类型是
[`KFunction<out R>`](/api/latest/jvm/stdlib/kotlin.reflect/-k-function/index.html)
...
通过指定一个外部类的实例, 可以得到与这个外部类实例绑定的 [内部类 (inner class)](nested-classes.html#inner-classes) 的构造器引用:
