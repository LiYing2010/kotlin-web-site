...
* The `kotlinOptions` DSL from the `KotlinCompile` 接口 that implements all Kotlin 编译 tasks. Use
  `KotlinCompilationTask<CompilerOptions>` instead.
* The `compilerOptions` 属性 with the `HasCompilerOptions` 类型 from the `KotlinCompiation` 接口. This DSL was
  inconsistent with other DSLs and 配置 the 相同的 `KotlinCommonCompilerOptions` 对象 as `compilerOptions` inside
  the `KotlinCompilation.compileTaskProvider` 编译 task, which was confusing.

  Instead, 我们 recommend 使用 the `compilerOptions` 属性 from the
  Kotlin 编译 task:
...
* The `kotlinOptions` DSL from the `KotlinCompilation` 接口.
* The `kotlinOptions` DSL from the `KotlinNativeArtifactConfig` 接口, the `KotlinNativeLink` 类,
  and the `KotlinNativeLinkArtifactTask` 类. 使用 the `toolOptions` DSL instead.
* The `dceOptions` DSL from the `KotlinJsDce` 接口. 使用 the `toolOptions` DSL instead.

For more 信息 on how to specify 编译器 选项 在 中 the Kotlin Gradle plugin, see [How to define 选项](gradle-compiler-options.md#how-to-define-options).

### 提高了 AGP 的最低支持版本 {id="bumped-minimum-supported-agp-version"}

从 开始 Kotlin 2.0.0, the minimum 支持 Android Gradle plugin 版本 is 7.1.3.

### 新的 Gradle 属性, 用于试用最新的语言版本 {id="new-gradle-property-for-trying-the-latest-language-version"}

Prior to Kotlin 2.0.0, 我们 had the 以下 Gradle 属性 to try out the 新的 K2 编译器: `kotlin.experimental.tryK2`.
现在 that the K2 编译器 is enabled by default 在 中 Kotlin 2.0.0, 我们 decided to evolve this 属性 into a 新的 form that
你 可以 使用 to try the 最新的 语言 版本 在 中 你的 项目: `kotlin.experimental.tryNext`. When 你 使用 this
属性 在 中 你的 `gradle.properties` 文件, the Kotlin Gradle plugin increments the 语言 版本 to one above the
default 值 for 你的 Kotlin 版本. 例如, 在 中 Kotlin 2.0.0, the default 语言 版本 is 2.0, so the
属性 配置 语言 版本 2.1.

This 新的 Gradle 属性 produces similar metrics 在 中 [构建 报告](gradle-compilation-and-caches.md#build-reports) as
before with `kotlin.experimental.tryK2`. The 语言 版本 配置 is included 在 中 the 输出. 例如:
...
To learn more about how to 启用 构建 报告 and their content, see [构建 报告](gradle-compilation-and-caches.md#build-reports).

### 构建报告的新的 JSON 输出格式 {id="new-json-output-format-for-build-reports"}

In Kotlin 1.7.0, 我们 引入了 构建 报告 to help track 编译器 性能. Over time, we've added more metrics to
make 这些 报告 even more detailed and helpful when investigating 性能 issues. 之前, the only 输出
格式 for a local 文件 was the `*.txt` 格式. 在 中 Kotlin 2.0.0, 我们 支持 the JSON 输出 格式 to make it even
easier to analyze 使用 other 工具.

To 配置 JSON 输出 格式 for 你的 构建 报告, 声明 the 以下 属性 在 中 你的 `gradle.properties` 文件:

```none
kotlin.build.report.output=json

// The 目录 to store 你的 构建 报告
kotlin.build.report.json.directory=my/directory/path
```

Alternatively, 你 可以 run the 以下 command:

```shell
./gradlew assemble -Pkotlin.build.report.output=json -Pkotlin.build.report.json.directory="my/directory/path"
``` 

Once 配置, Gradle 生成 你的 构建 报告 在 中 the 目录 that 你 specify with the 名称:
`${project_name}-date-time-<sequence_number>.json`.

Here's an example snippet from a 构建 报告 with JSON 输出 格式 that contains 构建 metrics and aggregated
metrics:
...
Prior to Kotlin 2.0.0, 如果 你 wanted to define a 共通 set of 注解 处理器 在 中 a separate Gradle 配置
and extend this 配置 在 中 kapt-specific 配置 for 你的 子项目, kapt would skip 注解
processing because it couldn't find any 注解 处理器. 在 中 Kotlin 2.0.0, kapt 可以 successfully detect that there
are indirect 依赖项 on 你的 注解 处理器.

As an example, for a 子项目 使用 [Dagger](https://dagger.dev/), 在 中 你的 `build.gradle(.kts)` 文件, 使用 the
以下 配置:
...
In this example, the `commonAnnotationProcessors` Gradle 配置 is 你的 共通 配置 for 注解
processing that 你 want to be used for all 你的 项目. 你 使用 the [`extendsFrom()`](https://docs.gradle.org/current/dsl/org.gradle.api.artifacts.Configuration.html#org.gradle.api.artifacts.Configuration:extendsFrom)
方法 to 添加 "commonAnnotationProcessors" as a 父配置(superconfiguration). kapt sees that the `commonAnnotationProcessors` 
Gradle 配置 has a 依赖项 on the Dagger 注解 processor. Therefore, kapt includes the Dagger 注解
processor 在 中 its 配置 for 注解 processing.

Thanks to Christoph Loy for the [实现](https://github.com/JetBrains/kotlin/pull/5198)!

### Kotlin Gradle plugin 不再使用已废弃的 Gradle 约定 {id="kotlin-gradle-plugin-no-longer-uses-deprecated-gradle-conventions"}

Prior to Kotlin 2.0.0, 如果 你 used Gradle 8.2 or higher, the Kotlin Gradle plugin incorrectly used Gradle conventions
that had been deprecated 在 中 Gradle 8.2. This led to Gradle reporting 构建 deprecations. 在 中 Kotlin 2.0.0, the Kotlin
Gradle plugin has been 更新 to no longer trigger 这些 deprecation 警告 when 你 使用 Gradle 8.2 or higher.

## 标准库 {id="standard-library"}

This release brings further stability to the Kotlin 标准库 and makes even more existing 函数 共通 for
all 平台:

* [枚举类型值的泛型函数的替代进入稳定版](#stable-replacement-of-the-enum-class-values-generic-function)
* [AutoCloseable 接口进入稳定版](#stable-autocloseable-interface)
* [共通 protected 属性 AbstractMutableList.modCount](#common-protected-property-abstractmutablelist-modcount)
* [共通 protected 函数 AbstractMutableList.removeRange](#common-protected-function-abstractmutablelist-removerange)
* [共通 String.toCharArray(destination)](#common-string-tochararray-destination-function)

### 枚举类型值的泛型函数的替代进入稳定版 {id="stable-replacement-of-the-enum-class-values-generic-function"}

In Kotlin 2.0.0, the `enumEntries<T>()` 函数 becomes [Stable](components-stability.md#stability-levels-explained).
The `enumEntries<T>()` 函数 is a replacement for the generic `enumValues<T>()` 函数. The 新的 函数 returns
a list of all enum entries for the given enum 类型 `T`. The `entries` 属性 for enum 类 was 之前 引入了
and 也 stabilized to replace the synthetic `values()` 函数. For more 信息 about the entries 属性,
see [What's new 在 中 Kotlin 1.8.20](whatsnew1820.md#a-modern-and-performant-replacement-of-the-enum-class-values-function).

> The `enumValues<T>()` 函数 is still 支持, but 我们 recommend that 你 使用 the `enumEntries<T>()` 函数
> instead because it has less of a 性能 impact. Every time 你 调用 `enumValues<T>()`, a 新的 array is 创建, whereas
> whenever 你 调用 `enumEntries<T>()`, the 相同的 list is returned each time, which is far more efficient.
>
{style="tip"}
...
In Kotlin 2.0.0, the 共通 [`AutoCloseable`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/-auto-closeable/)
接口 becomes [Stable](components-stability.md#stability-levels-explained). It allows 你 to easily close resources
and includes a couple of useful 函数:

* The `use()` 扩展 函数, which executes a given 代码块 函数 on the selected resource and then closes it down
  correctly, whether an 异常 is thrown or not.
* The `AutoCloseable()` 构造器 函数, which 创建 实例 of the `AutoCloseable` 接口.

In the example below, 我们 define the `XMLWriter` 接口 and assume that there is a resource that implements it.
例如, this resource could be a 类 that opens a 文件, writes XML content, and then closes it:
...
### 共通 protected 属性 AbstractMutableList.modCount {id="common-protected-property-abstractmutablelist-modcount"}
...
`protected` 属性 of the `AbstractMutableList` 接口 becomes 共通. 之前, the `modCount` 属性 was
available on each 平台 but not for the 共通 target. 现在, 你 可以 创建 custom implementations of `AbstractMutableList`
and access the 属性 在 中 共通 代码.

The 属性 keeps track of the number of structural modifications made to the 集合. This includes operations that
change the 集合 size or alter the list 在 中 a way that may cause iterations 在 中 progress to return incorrect results.

你 可以 使用 the `modCount` 属性 to register and detect concurrent modifications when implementing a custom list.

### 共通 protected 函数 AbstractMutableList.removeRange {id="common-protected-function-abstractmutablelist-removerange"}
...
`protected`函数 of the `AbstractMutableList` 接口 becomes 共通. 之前, it was available on each 平台
but not for the 共通 target. 现在, 你 可以 创建 custom implementations of `AbstractMutableList` and 覆盖 the
函数 在 中 共通 代码.
...
### 共通 String.toCharArray(destination) 函数 {id="common-string-tochararray-destination-function"}

This release 引入了 a 共通 [`String.toCharArray(destination)`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.text/to-char-array.html)
函数. 之前, it was only available on the JVM.

Let's compare it with the existing [`String.toCharArray()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.text/to-char-array.html) 函数.
It 创建 a 新的 `CharArray` that contains characters from the specified string. The 新的 common `String.toCharArray(destination)`
函数, however, moves `String` characters into an existing destination `CharArray`. This is useful 如果 你 already have
a buffer that 你 want to fill:
...
fun main() {
    ...
    // Convert the string and store it 在 中 the destinationArray:
...
bundled plugin included 在 中 你的 IDE. This means that 你 can't 安装 the plugin from JetBrains Marketplace anymore.

To 更新 to the 新的 Kotlin 版本, [change the Kotlin version](configure-build-for-eap.md#adjust-the-kotlin-version)
to 2.0.0 在 中 你的 构建 脚本.
