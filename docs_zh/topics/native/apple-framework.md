...
此外, Kotlin/Native 还有 [与 C 代码交互](native-c-interop.md) 功能.
还可以参考教程 [使用 Kotlin/Native 开发动态库](native-dynamic-libraries.md).
...
可以 [在 IDE 中](native-get-started.md) 运行 `linkNative` Gradle task 来构建库, 或执行以下控制台命令:
...
关于所有其它类型的映射, 详情请参见文档 [与 Swift/Objective-C 代码交互](native-objc-interop.md).
...
## 在 Objective-C 中使用代码 {id="use-the-code-from-objective-c"}
...
## Xcode 与 Framework 依赖项 {id="xcode-and-framework-dependencies"}

你需要配置 Xcode 项目来使用我们的 Framework.
具体的配置依赖于编译目标平台.

### 针对 macOS 编译目标的 Xcode 配置 {id="xcode-for-macos-target"}
...
详情请参见教程 [使用 Kotlin/Native 开发动态库](native-dynamic-libraries.md).
