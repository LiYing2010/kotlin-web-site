...
这个工具会分析 C 头文件, 并生成 C 的类型, 函数, 以及字符串到 Kotlin 的直接映射.
...
* 使用 [`.usePinned()`](https://kotlinlang.org/api/core/kotlin-stdlib/kotlinx.cinterop/use-pinned.html) 扩展函数,
  ...
  这个类型提供了一些有用的扩展, 例如 [`.addressOf()`](https://kotlinlang.org/api/core/kotlin-stdlib/kotlinx.cinterop/address-of.html),
  可以得到被固定的数组体的地址.
