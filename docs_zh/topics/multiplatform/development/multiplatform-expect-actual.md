...
> 本文描述预期声明与实际声明的语言机制.
> 关于使用平台相关 API 的各种方法的一般性建议, 请参见 [使用平台相关的 API](multiplatform-connect-to-apis.md).
>
{style="tip"}
...
例如, `iosMain` 作为中间源代码集, 在平台源代码集 `iosArm64Main` 和 `iosSimulatorArm64Main` 之间共用.
...
具体的示例请参见 [使用平台相关的 API](multiplatform-connect-to-apis.md#dependency-injection-framework).
...
> 预期类与实际类功能处于 [Beta 版](supported-platforms.md#general-kotlin-stability-levels).
