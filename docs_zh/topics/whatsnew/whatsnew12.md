...
<web-summary>阅读 Kotlin 1.2 发布说明, 包括新的语言特性, Kotlin Multiplatform, JVM, Native, JS 的更新, 以及对 Gradle 和 Maven 的构建工具支持.</web-summary>
...
关于创建跨平台项目的详细步骤, 请参见 [跨平台程序开发相关文档](get-started.topic).

## 语言层的其他特性 {id="other-language-features"}

### 在注解中使用数组字面值 {id="array-literals-in-annotations"}
...
### 顶级的属性和局部变量的延迟初始化(Lateinit) {id="lateinit-top-level-properties-and-local-variables"}
...
### 检查一个延迟初始化的变量是否已被初始化 {id="check-whether-a-lateinit-var-is-initialized"}
...
### 内联函数(Inline function) 的函数性参数的默认值 {id="inline-functions-with-default-functional-parameters"}
...
### 显式类型转换的相关信息可被用于类型推断 {id="information-from-explicit-casts-is-used-for-type-inference"}
...
### 智能类型转换的功能改进 {id="smart-cast-improvements"}
...
### 允许将 this::foo 简写为 ::foo {id="support-for-foo-as-a-shorthand-for-thisfoo"}

绑定到 `this` 的成员上的可调用的引用, 可以不用明确地指定接受者, 也就是说, `this::foo` 可以简写为 `::foo`.
在 lambda 表达式中, 如果你引用外层接受者的成员, 这种简化语法也使得可调用的引用更加便于使用.

### 破坏性变更: 对 try 代码段之后的智能类型转换进行警告 {id="breaking-change-sound-smart-casts-after-try-blocks"}
...
### 已废弃的功能: 覆盖 copy 函数的数据类 {id="deprecation-data-classes-overriding-copy"}
...
### 已废弃的功能: 在枚举值内的嵌套类型 {id="deprecation-nested-types-in-enum-entries"}

在枚举值内, 定义一个不是 `内部类(inner class)` 的嵌套类型, 这个功能已被废弃了. 因为会导致初始化逻辑中的错误.
在 Kotlin 1.2 中会产生编译警告, 在 Kotlin 1.3 中将会变为编译错误.

### 已废弃的功能: 以命名参数的方式对 vararg 参数传递单个值 {id="deprecation-single-named-argument-for-vararg"}
...
### 已废弃的功能: 泛型类的内部类继承 Throwable {id="deprecation-inner-classes-of-generic-classes-extending-throwable"}

泛型类的内部类如果继承自 `Throwable`, 在 throw-catch 语句中可能会破坏类型安全性, 因此这个功能已被废弃,
在 Kotlin 1.2 中会产生编译警告, 在 Kotlin 1.3 中将会变为编译错误.

### 已废弃的功能: 改变只读属性的后端域变量的值 {id="deprecation-mutating-backing-field-of-a-read-only-property"}
...
### Kotlin 标准库的 artifact 变更, 以及包分割问题 {id="kotlin-standard-library-artifacts-and-split-packages"}
...
### fill, replaceAll, shuffle/shuffled {id="fill-replaceall-shuffleshuffled"}
...
### BigInteger 和 BigDecimal 类型的操作符和转换 {id="operators-and-conversions-for-biginteger-and-bigdecimal"}
...
### 浮点数到位(bit)的转换 {id="floating-point-to-bits-conversions"}
...
### 正规表达式(Regex)变成了可序列化的对象(serializable) {id="regex-is-now-serializable"}

`kotlin.text.Regex` 类现在成为了 `Serializable`, 因此可以在对象序列化层级(serializable hierarchy)中使用正规表达式.

### 如果可能的话, Closeable.use 会调用 Throwable.addSuppressed {id="closeableuse-calls-throwableaddsuppressed-if-available"}
...
### 构造函数调用的正规化 {id="constructor-calls-normalization"}
...
### Java 默认方法(default method)调用 {id="java-default-method-calls"}
...
### 破坏性变更: 对于平台数据类型的 x.equals(null) 行为一致性 {id="breaking-change-consistent-behavior-of-xequalsnull-for-platform-types"}
...
### 破坏性变更: 对内联的扩展函数中值为 null 的平台数据类型变换的修正 {id="breaking-change-fix-for-platform-null-escaping-through-an-inlined-extension-receiver"}
...
### 默认支持 TypedArray {id="typedarrays-support-enabled-by-default"}
...
### 把警告作为错误来处理 {id="warnings-as-errors"}
